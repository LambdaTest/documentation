...
A `.evidence` file is a standard zip. `unzip -l ` lists it, and `unzip -p  ` prints one file without extracting the whole pack.
...
| `tests//test.md` | The test definition, that is, what was asked of the agent. It is **opaque**: the format references and hashes it, and never parses it. |
| `tests//result.yaml` | The structured per-step outcomes for that test. |
