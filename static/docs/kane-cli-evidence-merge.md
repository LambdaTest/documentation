...
|---|---|---|
| `--run-id ` | Run id for the merged pack, **required** | — |
| `-o, --out ` | Output path | `.testmuai/evidence/.evidence` |
| `--rules ` | Custom merge-rules file. Settings you omit keep their default value; the rule list itself is replaced | built-in defaults |
| `--on-collision ` | `error`, `prefer-first`, `prefer-latest`, or `discard` | `error` |
| `--title ` | Title for the merged run | first eligible pack's |
