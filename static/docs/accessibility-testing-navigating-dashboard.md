...
Web (Automated, Manual/DevTools, Scheduled) exclusion rules
...
Native Android (Real Device) exclusion rules
...
Native iOS (Real Device) exclusion rules
...
Same issues, different scores by page size
...
Example API response (test level and per-scan level)
