...
To run **many** tests as one execution — selected by path or `tags:` — use [`kane-cli testrun run`](/support/docs/kane-cli-testrun/).
...
| `--max-steps ` | integer | `30` | Maximum agent reasoning steps per test step. |
| `--timeout ` | integer | none | Hard kill timer per step, in seconds. |
| `--url ` | string | frontmatter `url:` / config `default_url` | Start URL for the first step. Overrides the frontmatter `url:` key and the configured `default_url`; bare domains are normalized to `https://`. |
...
| `--cdp-endpoint ` | string | none | Reuse an external Chrome over CDP. |
| `--ws-endpoint ` | string | none | LambdaTest / Playwright WebSocket endpoint. |
| `--global-context ` | path | `~/.testmuai/kaneai/global-memory.md` | Override the global context file. |
| `--local-context ` | path | `/.testmuai/context.md` | Override the local context file. |
| `--variables ` | JSON string | none | Inline variables JSON for this run. |
| `--variables-file ` | path | none | Load variables from a JSON file. |
| `--session-context ` | JSON string | none | Pre-loaded prior-run context for the agent. |
| `--username ` | string | profile | Basic-auth username. Skips OAuth for this run. |
| `--access-key ` | string | profile | Basic-auth access key. Skips OAuth for this run. |
| `--mode ` | `action` \| `testing` | profile or `testing` | Agent behaviour on auth walls and error pages. See [Run mode](#run-mode). |
...
| `--code-language ` | `python` \| `javascript` | `python` | Code-export target language. |
...
| `--name ` | string | none | Persist a session name. Must match `[a-zA-Z0-9_-]+`. |
| `--on-lock-conflict ` | `readonly` \| `fail` \| `wait` | none | Policy when another user holds the lock. See [Lock conflicts](#lock-conflicts). |
...
| `--retry-count ` | integer | `3` | Maximum replay restart attempts before falling back to a full re-author. |
| `--author` | flag | off | Skip the replay decision and author every step from scratch. |
| `--bug-detection ` | `off` \| `stop` \| `continue` | config value (`off`) | Detect product bugs while authoring: `stop` halts the run on a confirmed bug; `continue` records it and keeps going. Overrides `config set-bug-detection`. Authoring steps only — replay failures always investigate. See [Configuration](/support/docs/kane-cli-configuration/#bug-detection). |

Most flags have a frontmatter counterpart with the same name (with underscores). Where both are set, the CLI flag wins — except for `variables`, which the file owns; see [Writing test.md files](/support/docs/kane-cli-testmd/#variables).

Before any step runs, every `{{name}}` the authored steps reference must have a value, from the file's own `variables:` frontmatter, a variable file, `--variables-file`, or `--variables`. A name with no value stops the run before the browser starts, with exit `2` and a receipt naming the variable, the file waiting for its value, and the step. Replayed steps are not checked. See [Before a run](/support/docs/kane-cli-variables-and-context/#before-a-run-unresolved-variables).
...
- **Delete the output directory** — `rm -rf output-/` resets the cache for the next run.
...
- **`--retry-count `** — maximum restart attempts. Default `3`. After this many retries, kane-cli falls back to a full re-author.

Every failed replay is also **investigated automatically**: a failure record — the error, the page state at failure, and pointers into the step's console/network logs — is written into the run's [evidence pack](/support/docs/kane-cli-evidence-debugging/), so you can see *why* the replay broke, not just that it did.
...
A successful run writes everything it needs to replay next time into `output-/` next to the test file, where `` is the filename without `_test.md`:
...
`Result.md` is the human-readable run report. `.internal/` holds the recordings kane-cli replays on the next run. The whole `output-/` directory is safe — and recommended — to commit to git: it makes your tests reproducibly replayable on any teammate's machine and on CI.

For tests that `@import` helpers, kane-cli also writes one `helper-output-...` directory per call site next to the helper file. See [Composing tests with @import](/support/docs/kane-cli-testmd-composition/#helper-outputs).

### The evidence pack

Besides the output directory, every `testmd run` seals an [evidence pack](/support/docs/kane-cli-evidence/) — screenshots, per-step console/network logs, and failure records for the whole run — and copies it into `/.testmuai/evidence/`. In a terminal you're offered to open it in the browser viewer at the end of the run; replayed runs also publish their pack to your project's execution history in Test Manager. Do **not** commit `.testmuai/evidence/` to git — packs are run artifacts, not test sources.
...
The generated project lands at `output-/playwright--code/`. Code export requires a successful Test Manager upload to run, and only meaningful when a project and folder are configured.

For the configurable options (default language, validation toggle, persistent enable from the TUI), see [Configuration](/support/docs/kane-cli-configuration/#code-export).
...
If the runner cannot have Chrome at all, run the tests as a suite on the cloud grid instead. `kane-cli testrun run tests/ --remote` executes every `testmd run` on a HyperExecute macOS runner and returns the recordings and the evidence pack. See [Remote Runs](/support/docs/kane-cli-remote-execution/).
...
## `kane-cli testmd delete `

Removes the test source and its `output-/` directory:
...
| Flag | Description |
|---|---|
| `--username ` | Basic-auth username (skips OAuth) |
| `--access-key ` | Basic-auth access key |
...
- [Writing test.md files](/support/docs/kane-cli-testmd/) — file format, frontmatter, steps, variables.
- [Batch runs with testrun](/support/docs/kane-cli-testrun/) — run many tests as one execution with one evidence pack.
- [Evidence packs](/support/docs/kane-cli-evidence/) — what every run captures and how to view it.
- [Composition with @import](/support/docs/kane-cli-testmd-composition/) — reusable helpers across tests.
- [Configuration](/support/docs/kane-cli-configuration/) — persistent settings: Chrome profile, window size, code export defaults.
- [Test Manager integration](/support/docs/kane-cli-tms-integration/) — what happens during the upload.
