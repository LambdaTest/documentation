...
|-----------|------|-------------|
| mode | string | Defines where the pre-step commands will be executed.  Options: [local or remote](/support/docs/deep-dive-into-hyperexecute-yaml/#mode). Default `mode` is `remote`.|
...
| cache | boolean | It is used to cache the payload after all commands have executed. It is useful in cases like:  &nbsp;&nbsp;&nbsp; - If you want to made some modification in the payload at runtime by executing some set of commands.  &nbsp;&nbsp;&nbsp; - If the payload is fetched from git source. By caching the payload, git rate limiting can be avoided as the payload will only be fetched once per job with `cache: true`. |
...
| Parameter | Type | Description |
|-----------|------|-------------|
| mode | string | Defines where the post-step commands will be executed.  Options: [local or remote](/support/docs/deep-dive-into-hyperexecute-yaml/#mode). Default `mode` is `remote`. |
...
- Indicate the maximum number of retry attempts with `maxRetries: `.
...
- `commands`: actual commands that needs to run like `echo /output/output.log`
