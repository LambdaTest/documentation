...
> **Mobile is not supported in a batch run.** A `_test.md` with a mobile [`target:`](/support/docs/kane-cli-testmd/#mobile-target) (`emulator` / `simulator`) is rejected up front, before the suite runs. Run mobile tests one at a time with `kane-cli testmd run `.
...
| `--env ` | Environment (`prod` or `stage`) | active env |
