...
Each copy is named `Copy of `. If a run with that name already exists in the destination folder, a number is added, for example `Copy of Regression Suite (2)`. A run with the same name in a *different* folder does not count as a clash.
