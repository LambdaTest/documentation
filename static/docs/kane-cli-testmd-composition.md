...
- `` may be relative or absolute. Relative paths resolve against the directory of the **importing file**, not against your shell's working directory.
...
|---|---|
| Tests cannot be imported. Only files **not** ending in `_test.md` may appear after `@import`. | `cannot @import a test file: only helpers may be imported (got )` |
| No cycles. A helper cannot import a chain that eventually comes back to it. | `cyclic reference: a.md → b.md → a.md` |
| Imports must resolve. The target file must exist. | `@import path not found: ` |
| `optional` is allowed on `@import` only at the root file. | `intermediate-ref 'optional' is not supported in v1: :` |
| `@import` steps may not carry config other than `optional`. | `step config on @import may only contain 'optional': got ` |
...
Like the test's own `output-/`, `helper-output-...` directories are safe — and recommended — to commit to git.
