...
| Option | Description |
|--------|-------------|
| `-P, --port ` | Specify a custom port number for the server (default: 49152) |
| `--fetch-results [filename]` | Fetch test results and save to a JSON file (default: results.json) |
| `--buildName ` | Specify a custom build name for the test run |
| `--scheduled ` | Specify the schedule ID for scheduled test runs |
| `--show-render-errors` | Show render errors from SmartUI build |
| `--userName ` | Override TestMu AI username (overrides environment variable) |
