...
| Variable | Effect |
|---|---|
| `ROOK_VERSION=` | Install a specific build instead of the newest available build. |
| `ROOK_PREFIX=` | Install somewhere other than `~/.testmuai/rook`. |
| `ROOK_GITHUB_TOKEN=` | Authenticate without GitHub CLI. |
...
Download a `rook-.tar.gz` archive from a `cli-` release tag. With GitHub CLI:
