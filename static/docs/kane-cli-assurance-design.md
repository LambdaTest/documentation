...
| `--phase ` *(0.7.1)* | Enter the design at a specific phase (`grounding`, `acs`, `scenarios`, `wiring`, `tests`), re-seeded from the committed earlier phases. Missing predecessors prompt interactively; in agent mode the run exits `2` with the runnable commands in `next` |
...
| `--answer =` *(0.7.1)* | With `--resume --mode agent`: answer a specific pending question by id — `=`, repeatable |
