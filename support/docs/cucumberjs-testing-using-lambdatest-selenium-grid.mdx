...
>**Note:** All the code samples in this documentation can be found in the [<BrandName />'s Repository on GitHub](https://github.com/<BrandName />/NodeJs-Cucumber-Selenium). You can either download or clone the repository to quickly run your tests.
...
**Step 1:** Clone the <BrandName />’s [NodeJs-Cucumber-Selenium repository](https://github.com/<BrandName />/NodeJs-Cucumber-Selenium) and navigate to the code directory as shown below:
```bash
git clone https://github.com/TestMu AI/NodeJs-Cucumber-Selenium
...
Feature: Automate a website
    Scenario: perform click events
      When visit url "https://testmu ai.github.io/sample-todo-app"
...
module.exports = function() {
  ...
  this.When(/^visit url "([^"]*)"$/, function (url, next) {
    this.driver.get('https://testmu ai.github.io/sample-todo-app').then(next);
  });
