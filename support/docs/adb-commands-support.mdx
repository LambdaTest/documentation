...
keywords: ['appium', 'java', 'lambdatest java', 'framework on lambdatest', 'testng', 'app testing', 'real devices', 'adb commands', 'android debug bridge']
---
...
All these commands are supported on both Real and Virtual Devices except for the [**adb shell dumpsys**](/support/docs/adb-commands-support/#adb-shell-command) command, which is not supported for Virtual Device.
</Note>
...
<VirtualDeviceTag value="Virtual Device" />
  ...
  **NOTE**
  ...
  The **-c** is required in this command
  </Note>
  ```python
...
<VirtualDeviceTag value="Virtual Device App" />
