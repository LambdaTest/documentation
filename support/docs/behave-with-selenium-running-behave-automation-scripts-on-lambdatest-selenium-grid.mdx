...
>**Note:** All the code samples in this documentation can be found in the [<BrandName />'s Behave Repository on GitHub](https://github.com/<BrandName />/Python-Behave-Selenium). You can either download or clone the repository to quickly run your tests.
...
**Step 1:** Clone the <BrandName />’s [Python-Behave-Selenium repository](https://github.com/<BrandName />/Python-Behave-Selenium) and navigate to the code directory as shown below:
```bash
git clone https://github.com/TestMu AI/Python-Behave-Selenium
...
```
**Step 4:** Install the [required packages](https://github.com/<BrandName />/Python-Behave-Selenium/blob/master/requirements.txt) from the cloned project directory:
```bash
...
def step(context):
    before_all(context)
    context.driver.get('https://testmu ai.github.io/sample-todo-app/')
