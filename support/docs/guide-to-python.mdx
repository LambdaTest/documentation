...
Now we are all set to execute the first Python test on <BrandName /> Selenium Grid. Checkout the sample code below. This is a simple python code that uses <BrandName /> Selenium grid to open up Google. You can also download it from our [GitHub repo](https://github.com/<BrandName />/python-selenium-sample).
