...
<GitHubButton url="https://github.com/LambdaTest/LT-appium-nodejs-webdriverio" />
</Check>
...
```javascript specs/android-test.js
describe("Proverbial APK", () => {
  it("Changes color", async () => {
    var color = await $("id=color");
    await color.waitForDisplayed({ timeout: 30000 });
...
```javascript ios-sample/ios-single.conf.js lines highlight={2-3,6,17}
exports.config = {
...
```javascript android-sample/android-single.conf.js lines highlight={2-3,6}
exports.config = {
  user: process.env.LT_USERNAME || "YOUR_USERNAME",
  key: process.env.LT_ACCESS_KEY || "YOUR_ACCESS_KEY",
