...
sidebarTitle: "iOS – What We Cover"
---

import { BrandName } from "/snippets/BrandName.mdx";

# <BrandName /> Accessibility iOS – What We Cover

<Note>
This document describes <BrandName /> Accessibility coverage for iOS. For platform or accessibility support, contact <BrandName /> support.
</Note>
...
This document describes **what <BrandName /> Accessibility iOS covers**: a **full WCAG rules reference** (100% for iOS), a **checklist** of rules the platform can check, **how to use** the accessibility testing platform to run scans and view results, and **how to check** / **how to do** (remediation) for **native iOS apps**. It is intended for teams who:

- Run accessibility scans on iOS apps via <BrandName /> Accessibility (e.g. <BrandName /> Accessibility DevTools or the accessibility testing platform with supported engines).
...
Below is a consolidated checklist by WCAG version, plus an **iOS-specific rules** table that maps to the rules <BrandName /> Accessibility iOS can report. **Level** is A, AA, or AAA; **Impact** is typical severity (Critical, Serious, Moderate, Minor). Use **Section 4 – How to use the platform to test (iOS)** to run scans.
...
### 3.4 iOS-specific rules (<BrandName /> Accessibility iOS)

These map to the rules that <BrandName /> Accessibility iOS can report. Use this table for **what we cover** and quick remediation direction.
...
<Note>
Exact rule set depends on your <BrandName /> Accessibility version. Check <BrandName /> docs for the latest iOS rule list.
</Note>
...
Use the <BrandName /> Accessibility platform to run automated accessibility scans on your **native iOS app**. Follow these steps to test covered items:

| Step | How to use the platform to test |
|------|-------------------------|
| **1. Access the platform** | Log in to <BrandName /> and open the **Accessibility** (or **<BrandName /> Accessibility**) section. Ensure you have access to **iOS** device or simulator for testing. |
...
<Tip>
Combine automated scans with **manual testing** (VoiceOver, Switch Control, Dynamic Type) and the **What we do NOT cover** document for full WCAG coverage and compliance.
</Tip>
...
- Run <BrandName /> Accessibility scan on the iOS app and review violations for missing labels, duplicate labels, and role/traits.
...
- **Support:** For <BrandName /> platform, accessibility coverage, or device issues contact <BrandName /> support.
