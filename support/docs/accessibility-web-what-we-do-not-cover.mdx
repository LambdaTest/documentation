...
sidebarTitle: "Web – What We Do Not Cover"
---

import { BrandName } from "/snippets/BrandName.mdx";

# <BrandName /> Accessibility Web – What We Do NOT Cover
...
This document describes **what <BrandName /> Accessibility Web does NOT cover**: the **manual tests** (and assistive-technology testing) that **you need to perform** for **full WCAG rules coverage and compliance**. <BrandName /> Accessibility automated scanning on web typically covers a subset of accessibility issues; the rest require human judgment, keyboard/screen-reader testing, and media/layout checks.

- **Full rules reference:** Section 2 lists **100% of WCAG 2.0, 2.1, and 2.2** success criteria (Level A, AA, and AAA) for web.
- **What we do not cover:** Section 3 is a **checklist** of criteria that require **manual testing** (or assistive-technology testing) because <BrandName /> Accessibility does not support them or only partially supports them.
...
The following criteria **require manual testing** (or assistive-technology testing) because <BrandName /> Accessibility does not support them or only partially supports them.
...
- **Support:** For <BrandName /> platform, accessibility coverage, or scan issues contact <BrandName /> support.
