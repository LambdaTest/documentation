...
When a build is executed in the same branch as the **Baseline Branch**, SmartUI will compare it against the latest approved baseline build. If some screenshots (or variants) that were present in the baseline are missing from the current build, a `Missing Screenshots` warning ![Smart Visual Testing](/assets/images/smart-visual-testing/missing-ss/Validation.webp) will be displayed.

![Smart Visual Testing](/assets/images/smart-visual-testing/missing-ss/Tooltip-missing.webp)
