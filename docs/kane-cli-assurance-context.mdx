---
title: "How to Build the Kane CLI Context Graph"
sidebarTitle: "Context Graph"
description: "Snapshot requirement documents into a local, content-addressed store and extract use-cases with an AI agent using kane-cli context."
...
Headless extraction (`--mode agent|ci|override`), the NDJSON event stream, exit codes, and the pause/resume contract are documented in [Agents and CI](/docs/kane-cli-assurance-automation/). A pipeline that works on a shared store clones it once, pulls before each run and pushes after: the sync verbs speak the same stream, see [the sync verbs on the stream](/docs/kane-cli-assurance-automation/#the-sync-verbs-on-the-stream), and the CI shape is in [CI/CD](/docs/kane-cli-cicd/#a-shared-context-store-in-ci).
...
- [Sharing the context graph with your team](/docs/kane-cli-assurance-sharing/) — one location; publishing, taking your teammates' records, cloning, and what happens when two people change the same thing.
