...
| Mismatched Label Text  <NewTag  value="New" /> | 2.5.3 | A | Serious | The accessible name doesn't contain the text visibly rendered on the control, so Voice Control users saying what they see can't activate it. Make the accessible name contain (ideally lead with) the visible text. | [Open rule](/support/docs/accessibility-ios-rule-mismatched-label-text/) |
| Misplaced Field Label  <NewTag  value="New" /> | 3.3.2 | A | Moderate | The accessible name contains the visible text but doesn't begin with it, slowing Voice Control matching and burying the word sighted users see. Reorder so the visible text leads the accessible name. | [Open rule](/support/docs/accessibility-ios-rule-misplaced-field-label/) |
