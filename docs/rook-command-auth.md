...
<img loading="lazy" src={require('../assets/images/rook/commands/rook-command-auth.png').default} alt="Rook auth command help showing the status subcommand" width="1556" height="1466" className="doc_img"/>
...
2. Confirm the account identity and valid status.
3. If invalid, use <code>/login</code>.
4. Run the status check again.

## State and privacy

The status check reads the stored token and verifies it remotely. It does not print the token or change project data.
