...
title: AI Coding Agents Use Cases
...
site_name: TestMu AI
slug: kane-cli-with-ai-coding-agents/
canonical: https://www.testmuai.com/support/docs/kane-cli-with-ai-coding-agents/
---
...
<script type="application/ld+json"
      dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "itemListElement": [{
          ...
          "item": "https://www.testmuai.com/support/docs/kane-cli-with-ai-coding-agents/"
        }]
      }) }}
></script>
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/ai-coding-agents-architecture.svg" alt="Architecture of AI coding agents working with Kane CLI: any builder such as Claude Code, Cursor, Gemini CLI, Codex CLI, Kiro, Lovable, Antigravity, or GLM reads one skill file, runs a single kane-cli command, drives a real Chrome session, and gets back an NDJSON stream and an evidence pack it can act on" className="doc_img img_center" width="1000" height="563" />
...
<VideoEmbed src="https://www.youtube.com/embed/cdiPVU2JBEs" title="Claude Code x Kane CLI" />
...
6. **Save and commit the ones worth keeping.** `kane-cli generate --save --req <id>` writes them as runnable `_test.md` files, and `--refine --req <id>` adjusts coverage before saving. A committed `checkout_test.md` carries its own frontmatter, `mode: testing`, `max_steps: 25`, `timeout: 180`, and an `app_url` variable, with steps that assert the Pro total shows `$19.00`.
...
<VideoEmbed src="https://www.youtube.com/embed/u5vi-9Sn97A" title="Lovable x Kane CLI" />
...
<source src= {require('../assets/videos/kane-cli/agents/cursor-x-kane-cli.mp4').default} type="video/mp4" />
</video>
...
<video class="right-side" width="100%" controls id="vid">
<source src= {require('../assets/videos/kane-cli/agents/gemini-cli-x-kane-cli.mp4').default} type="video/mp4" />
</video>
...
<video class="right-side" width="100%" controls id="vid">
<source src= {require('../assets/videos/kane-cli/agents/kiro-x-kane-cli.mp4').default} type="video/mp4" />
</video>
...
<video class="right-side" width="100%" controls id="vid">
<source src= {require('../assets/videos/kane-cli/agents/antigravity-x-kane-cli.mp4').default} type="video/mp4" />
</video>
...
<video class="right-side" width="100%" controls id="vid">
<source src= {require('../assets/videos/kane-cli/agents/glm-x-kane-cli.mp4').default} type="video/mp4" />
</video>
...
3. It moves the file into your repo, typically under `tests/`, and commits it alongside its `output-<stem>/` recording directory, which is commit-safe by design.
...
6. It commits the updated test files and their `output-<stem>/` directories so teammates and CI replay the corrected recordings.
