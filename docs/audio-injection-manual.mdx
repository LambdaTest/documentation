...
keywords: ['audio injection', 'manual testing', 'App Live', 'microphone input', 'voice commands', 'live input', 'real devices', 'testmu ai audio testing']
...
"og:description": "Manually test microphone-driven flows on real Android and iOS devices in App Live by injecting pre-recorded audio files or streaming live input from your system mic."
---

import { BrandName } from "/snippets/BrandName.mdx";

**Audio Injection** lets you simulate microphone input on real Android and iOS devices during a manual session. Use it to test speech-to-text, voice commands, voice assistants, in-app recording, KYC voice verification, and any other microphone-dependent feature — without speaking into a physical mic.

With <BrandName />, you can either inject a **pre-uploaded audio file** or stream **Live Input** directly from your system microphone into the device under test.
...
- [Audio Injection on Real Devices (Automation)](/docs/audio-injection/) — Inject audio via Appium / Selenium tests
- [Browser Testing on Real Devices](/docs/real-device-testing-overview/)
