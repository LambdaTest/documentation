...
| `--mode <mode>` | Ask policy for headless runs: `agent` \| `ci` \| `override` — bare non-TTY exits `2`. See [Automation](/support/docs/kane-cli-assurance-automation/) |
...
| `--resume <sid>` | Resume a paused session ([sessions](/support/docs/kane-cli-assurance-context/#sessions)) |
