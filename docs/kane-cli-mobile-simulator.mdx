---
title: "How to Set Up iOS Simulator for Kane CLI"
sidebarTitle: "iOS Simulator Setup"
description: "Set up Apple's iOS Simulator so Kane CLI can run mobile tests on a virtual iOS device on macOS Apple Silicon: install Xcode 16+ and point the CLI tools at it."
keywords: ['kane cli ios simulator', 'ios simulator setup', 'xcode simctl', 'kane cli doctor', 'mobile app testing']
"og:description": "Set up Apple's iOS Simulator so Kane CLI can run mobile tests on a virtual iOS device on macOS Apple Silicon: install Xcode 16+ and point the CLI tools at it."
---

Set up Apple's iOS Simulator once, and Kane CLI can run mobile tests against it. This guide targets **macOS on Apple Silicon (arm64)**, the only supported host for this release. See [Mobile Testing](/docs/kane-cli-mobile/) for the full picture.
