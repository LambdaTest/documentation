...
<script type="application/ld+json"
  dangerouslySetInnerHTML={{ __html: JSON.stringify({
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": "https://www.testmuai.com/support/docs/author-your-first-mobile-app-test/"
    },
    "headline": "Author Your First Mobile App Test",
...
To capture an action that is hard to describe, use **Manual Interaction**. While the session is in Authoring Steps, perform the step directly on the device, and KaneAI records it and creates the matching automation step. For the full gesture and platform matrix, see the [KaneAI Manual Interaction](/support/docs/kaneai-manual-interaction/) guide.
