...
1. Navigate to the Accessibility section on your <BrandName /> dashboard.
...
The <BrandName /> URL Crawler automatically discovers URLs across a site so you do not have to enter each page by hand, which matters most for large sites with many pages.
...
1. Open the <BrandName /> Accessibility Testing Dashboard.
