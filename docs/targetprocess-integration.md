...
TargetProcess uses boards to visualize data. The dashboard URL would be the link to your customized board that you would create to track any project, tasks, bugs etc. We will be using the highlighted User Story for demonstration.
...
Go to the Access Tokens tab and Add a new token.
...
**Step 1**: Go for any of the tests from the left navigation menu. For the demo, we will be taking Real Time Test option.

**Step 2**: Present a URL of the web app you need to test in the dialog box. After that, select any configuration for the browser and operating system of your choice & hit ‘Start’.
...
**Step 4**: After a screenshot is captured, you can annotate any issue or task with a built-in image editor. Once you are done highlighting the bug, click on the button that says Mark as Bug.
...
**Step 5**: After clicking on Mark as Bug button a form specific to TargetProcess would open up. Fill in the fields as per your requirement, and create the issue.

> **You can specify**:

- The project you want to assign an issue.
- The issue type is whether it is a bug or a task.
...
**Step 3**: Under **“My Integrations“**, you’ll find the TargetProcess Integration. Click on the Remove button to uninstall the integration with LambdaTest and TargetProcess.
