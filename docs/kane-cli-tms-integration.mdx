---
title: "How to Upload Kane CLI Tests to Test Manager"
sidebarTitle: "Test Manager Integration"
description: "Kane CLI uploads every session to TestMu AI Test Manager as a test case. Learn the upload pipeline, project and folder selection, code export, and share links."
keywords: ['kane cli tms', 'kane cli test manager', 'kane cli upload', 'kane cli code export', 'kaneai', 'testmu ai']
"og:description": "Kane CLI uploads every session to TestMu AI Test Manager as a test case. Learn the upload pipeline, project and folder selection, code export, and share links."
---
...
The run's sealed [evidence pack](/docs/kane-cli-evidence/) rides along with the upload, and the pack records who ran the test (user name/email) and the share link. **Replayed `testmd` runs publish evidence too**: a pure replay skips the authoring upload pipeline entirely, but its sealed pack is still published to the test's own project, so the dashboard shows execution history even for cache-replayed runs. The same applies to `testrun` executions.
