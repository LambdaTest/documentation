---
title: "How to Use Variables in Kane CLI"
...
|-------|----------|------|---------|-------------|
| `value` | Yes | string | — | The variable's value. A number is accepted and loaded as its string (`8080` becomes `"8080"`), while a boolean, object or array is not a value. Entries without `value` are ignored. |
...
### Before a run: unresolved variables {#before-a-run-unresolved-variables}
