...
<script type="application/ld+json"
      dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "itemListElement": [{
          ...
          "position": 3,
          "name": "Use Cases",
          "item": "https://www.testmuai.com/support/docs/kane-cli-use-cases/"
        },{
          "@type": "ListItem",
          "position": 4,
          "name": "API Testing",
          "item": "https://www.testmuai.com/support/docs/api-testing-with-kane-cli/"
        }]
      }) }}
></script>
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/smoke-test-an-endpoint.svg" alt="Smoke testing an endpoint: a single plain-English kane-cli run command calls GET /health and asserts both the 200 status and the 'ok' field in the response body" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/auth-token-chaining.svg" alt="Auth-token chaining flow: a POST to /auth/login stores the returned token, which is then reused automatically in the GET /orders, POST /items, and GET /me requests" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/schema-and-field-validation.svg" alt="Schema and field validation: a GET /users/42 response is checked field by field against the expected shape, and the run fails at the point where a missing role field breaks the contract" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/server-side-business-logic.svg" alt="Server-side business logic: a cart of two books and a SAVE10 coupon is posted, and the assertion is made against the total the server computed rather than anything shown on screen" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/negative-path-401.svg" alt="Negative path testing: a deliberately invalid token is sent to GET /api/orders and the auth gate bounces it with a 401, so the rejection is what makes the run pass" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/sla-and-response-time.svg" alt="Response-time and SLA gates: a request that returns a correct payload can still fail the run when it breaches the response-time budget, catching the slow path before a user does" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/migrate-an-existing-curl.svg" alt="Migrating an existing curl command: a long curl invocation full of flags becomes a plain-English POST /orders instruction that makes the same call and ends with a pass or fail verdict" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/input-validation-boundaries.svg" alt="Input validation boundaries: an empty email is posted to /signup and rejected with a 400, then a valid email is posted and accepted with a 201, proving both sides of the rule in one flow" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/cross-endpoint-consistency.svg" alt="Cross-endpoint data consistency: an order created by POST /orders is read back by GET /orders/42 and listed by GET /users/7/orders, and any mismatch between the three fails the run" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/filter-catalog-then-act.svg" alt="Filter a catalog then act: a filtered GET /products call returns a list, the first result's id is stored as a variable, and that variable is used in the follow-up POST /cart instead of a hardcoded id" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/create-a-record-seed-data.svg" alt="Create a record as seed data: the payload is supplied as variables so the same POST /records seeding flow runs unchanged against dev, staging, and production" className="doc_img img_center" width="1000" height="563" />
...
<img loading="lazy" src="/support/assets/images/kane-cli/sketchnotes/multi-step-api-transaction.svg" alt="Multi-step API transaction: create, update, verify, delete, and confirm-gone run as five chained steps, with one stored order id flowing through every request so the run starts clean and ends clean" className="doc_img img_center" width="1000" height="563" />
