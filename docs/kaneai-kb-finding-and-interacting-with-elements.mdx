...
sidebarTitle: "Find & Interact With Elements"
---
...
### Scroll Until Element on Mobile

On mobile, elements are often below the fold or deep within long scrollable pages. The **Scroll Until Element** feature scrolls automatically until the target you describe becomes visible — no selectors or manual scroll logic required.

Reference the element by what is visibly present on the screen:

- Visible text (e.g., `Login button`)
- Element type (e.g., `input field`)

<Note>
Natural language commands do **not** support technical selectors such as IDs, XPaths, or CSS attributes on mobile. Always describe what is visibly present on the screen.
</Note>

Write the command in action-oriented language — state what to find and what to do once found:
...
scroll inside the dropdown until New option and select it
```

#### Mobile Scroll Limitations

**Mobile app authoring:**

- **Scroll limit** — a maximum of 20 scroll attempts is performed. If the element is not found within 20 scrolls, the action fails.
...
- **Directional extremes** — `scroll to top` and `scroll to bottom` are not supported.
...
#### Troubleshooting Mobile Scroll

| Behavior | Cause | Fix |
|---|---|---|
| Maximum scroll limit reached | Element is beyond 20 scrolls or not present | Verify the element appears within 20 scrolls |
| Element not found (mobile browser) | Non-interactable or plain-text element | Target a button, link, or input field instead |
| Scroll timeout (app agent) | Element is inside a nested container | Use manual scroll steps if the element is not in the main viewport |
| Command not recognized | IDs, XPaths, or attributes used | Rewrite using visible text or element type (e.g., `button`) |
