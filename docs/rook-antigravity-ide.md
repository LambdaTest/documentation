...
The readiness prompt below uses a named skill request; the CLI's `/skills` commands are not instructions to type in the IDE terminal. Keep command review enabled. A generated plan or an IDE artifact is not proof that a Rook test ran.

## Test Your Agent Through Prompts {#approve-and-run-a-bounded-test}
...
```text
Use the rook skill from .agents/skills/rook. Inspect this repository and my Rook setup.
...
## Review the Actual Result {#review-rook-evidence-alongside-the-ide}
...
Filter **Acceptance criteria** and use the **Evidence** drawer for **Request**, **Response**, **Verdict**, or **Artefacts**. This page-only CommerceCare demo capture illustrates Rook's result view; it is not an Antigravity IDE screenshot or evidence that this integration was executed. See the [earlier local layout](/support/docs/rook-web-ui/#earlier-local-ui) if your CLI still has scrolling sections.

<img loading="lazy" src={require('../assets/images/rook/rook-local-result.png').default} alt="Local CommerceCare result showing failed and unverifiable criteria for the saved demo" width="1440" height="900" className="doc_img"/>

### Hosted Web UI

For uploaded results, choose **Verdict** or **Artefacts** in the **Evidence** drawer. This separate hosted triage example is illustrative Rook evidence, not an IDE-specific test capture.
...
For missing observations or incomplete runs, see [results and evidence](/support/docs/agent-assurance-results-and-evidence/). Use [both UI walkthroughs](/support/docs/rook-web-ui/) for the full navigation flow.
