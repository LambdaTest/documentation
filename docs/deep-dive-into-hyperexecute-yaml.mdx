...
sidebarTitle: "Configure Every YAML Setting"
...
## Quick index of YAML keys

Use the categorized tables below to scan every supported HyperExecute YAML key and jump straight to its detailed explanation further down this page.

### Mandatory Parameters

| Key | Type | Description|
|-----|------|------------|
| [version](#version) | String | The version of HyperExecute YAML you are using. | 
| [runson](#runson) | _linux_, _mac_, or _win_ | The OS on which you will run your Test. Can provide [`${matrix.os}`](#matrix) as well to run a multi OS job.|
| [pre](#pre) | List of Strings | The actions that are performed before test execution. |

### AutoSplit Mode Parameters

| Key | Type | Description|
|-----|------|------------|
| [autosplit](#autosplit) | Boolean | Autosplit utilizes an AI-based algorithm to intelligently distribute your discovered tests among concurrent virtual machines (VMs). It takes into account the previous test runs and statuses to achieve the most optimal distribution of tests. By doing so, it maximizes the efficiency and effectiveness of your test execution process. <br /> Default value is `false`. |
| [concurrency](#concurrency) | Integer | Indicates the number of concurrent sessions on HyperExecute. |
| [testDiscovery](#testdiscovery) | Map  | This key is used to locate or discover relevant tests via class names, filters, file names, etc. |
| [testRunnerCommand](#testrunnercommand) | String | A command used to run a single test entity in isolation.This entity could be a file , module, feature or scenario. |

### Matrix Mode Parameters

| Key | Type | Description|
|-----|------|------------|
| [matrix](#matrix) | String | A matrix allows you to create multiple tasks by performing variable substitutions in a single job definition. |
| [combineTasksInMatrixMode](#combinetasksinmatrixmode) | Boolean | This flag is passed in matrix mode to run the (matrix-multiplied) combinations as scenarios in the specified number of HyperExecute machines, instead of 1 machine per matrix-combination.|
| [testSuites](#testsuites) | String | A command to run the tests that were mentioned in the scenario key for matrix based test execution. |

### Hybrid Mode Parameters

| Key | Type | Description|
|-----|------|------------|
| [parallelism](#parallelism) | Integer | Parallelism defines number VMs to be spawned in case of [`hybrid`](/support/docs/hyperexecute-hybrid-strategy/) mode, if the user is not defining the parallelism then he must define platform specific parallelism (win , mac and linux). If both are defined the preference will be given to platform base parallelism |
| [macParallelism](#macparallelism) | Integer | It defines number of mac VM to be spawned for job. |
| [winParallelism](#winparallelism) | Integer | It defines number of win VM to be spawned for job. |
| [linuxParallelism](#linuxparallelism) | Integer | It defines number of linux VM to be spawned for job. |
| [testRunnerCommand](#testrunnercommand) | String | A command used to run a single test entity in isolation.This entity could be a file , module, feature or scenario. |
| [macTestRunnerCommand](#mactestrunnercommand) | String | Test runner command for MAC. | 
| [winTestRunnerCommand](#wintestrunnercommand) | String | Test runner command for Windows. |
| [linuxTestRunnerCommand](#linuxtestrunnercommand) | String | Test runner command for Linux. |

### Basic Parameters

| Key | Type | Description|
|-----|------|------------|
| [cacheKey](#cachekey) | String | File which can be used to generate unique key for cache. Example : package.json |
| [cacheDirectories](#cachedirectories) | List of Strings | Dependency directories that are cached to improve the test execution speed. |
| [env](#env) | Map | No |
| [runtime](#runtime) | Map | Install any dependent language/framework with any specific version. |
| [retryOnFailure](#retryonfailure) | Boolean | If set to true, then it will retry tests based on `maxRetries` key as defined below. <br /> Default value is `false`. |
| [maxRetries](#maxretries) | Integer | The maximum number of times your scenarios can be retried. <br /> If `retryOnFailure` is true then Default value will be 2. Value Range 1 to 5.|
| [post](#post) | List of Strings | The actions that are performed after test execution. |
| [report](#report) | Boolean | To generate the test report at the end of the job using the framework generated reports, set this value to true. We also need to set partialReports for generating report |
| [partialReports](#report) | Map | Configurations related to generate report. |
| [errorCategorizedOnFailureOnly](#errorcategorizedonfailureonly) | Boolean | Control the behavior of error categorization generated on your tests|
|[errorCategorizedReport](#errorcategorizedreport) | Map, Boolean | Generate a RCA report for your failed tests.|
| [jobLabel](#joblabel) | List of Strings | Add tags or labels to jobs. This allows you to search your jobs using the labels or tags assigned to them. |
...
| Key | Type | Description|
|-----|------|------------|
| [mergeArtifacts](#mergeartifacts) | Boolean | Merge all the artifacts generated by each task. |
| [uploadArtefacts](#uploadartefacts) | Map | The uploadArtifacts key contains the path and the name of the file which you want to download later as an artifact when job finishes.|
| [captureCSVResult](#capturecsvresult) | Boolean |If this directive is enabled, it will generate the performance stats artifact having aggregated information for the time taken by Selenium Commands. |
| [skipArtifactStageIfNoTest](#skipartifactstageifnotest) | Boolean |When enabled, it will mark artifact stage as skipped when there is no selenium/cdp test executed in the task. |
|[linkValidity](#linkvalidity) | Integer | Customize the duration of validity for report and artifact URLs generated after job completion. |
| [stripParentDirectory](#stripparentdirectory) | Boolean | When `mergeArtifacts` is true, providing this flag will strip the parent directory from all the downloaded artifacts and place the artifacts in base path. | 
| [generateArtifactAfterEveryStage](#generateartifactaftereverystage)  | Boolean | With this flag `artifacts` will be generated after every `stage`, without this flag artifacts are generated after every task and avoid any overrides for artifacts of the same name. |
| [taskIdentifierInNonConflictingArtifacts](#taskidentifierinnonconflictingartifacts) | Boolean |This flag when set to **true** specifies that the task ID should be included in the non-conflicting artifacts. |

#### Timeout

| Key | Type | Description|
|-----|------|------------|
| [globalTimeout](#globaltimeout) | Integer | It sets the max duration (1-150 mins) for a HyperExecute job. It terminates a job exceeding this limit. Default is 90 mins, but can be adjusted based on your requirements.|
| [testSuiteTimeout](#testsuitetimeout) | Integer  | It is used to set the timeout on all scenario stages inside a task. If defined, your complete test suite should get executed within this time. Its max value is 150 minutes.|
| [testSuiteStep](#testsuitestep) | Integer | It is used to timeout individual scenario  stages in a task. Each scenario in each task should get completed before this time. |

#### Tunnel

| Key | Type | Description|
|-----|------|------------|
| [tunnel](#tunnel) | Boolean | If set to true , tunnel will be spawn on run time , which connects system where we are running the cli to VM where tests are getting executed. |
| [tunnelOpts](#tunnelopts) | Map | All the configuration related to tunnel will be mention here. Please note tunnelOpts will work along with either `tunnel` or `tunnelNames`. |
| [tunnelNames](#tunnelnames) | List of Strings | List of tunnels name which has been registered and running with HyperExecute. |

#### Other Advanced Parameters

| Key | Type | Description|
|-----|------|------------|
| [globalPre](#globalpre) | Map | This flag runs once before all tasks start, used for global setup such as installing dependencies or configuring environments. |
| [globalPost](#globalpost) | Map | This flag runs once after all tasks finish, used for global cleanup such as removing logs or finalizing reports. |
| [failFast](#failfast) | Map | Flag to fail a job faster if there are `x` consecutive failures. |
| [differentialUpload](#differentialupload) | Map | Minimize the time of upload of code. |
| [background](#background) | Map |It is used to trigger long-running Application Servers tasks like running WebApps or databases |
| [retryOptions](#retryoptions) | Array | Retries the test case if found specific errors in the logs. |
| [base](#base) | Map | Inherit the configurations of a base YAML file into your parent file. |
| [sourcePayload](#sourcepayload) | Map | You can use this to define the source of your test scripts so that the scripts can directly be fetched from your git directly for execution on HyperExecute. |
| [hostsOverride](#hostsoverride) | List of Maps |If you want to add custom domain mappings in local DNS entry. |
| [frameworkStatusOnly](#frameworkstatusonly) | Boolean | Set status of scenario based on it’s tests status / remarks. |
| [vars](#vars) | Map |  Here we define all the variable which can be refer in other parts of yaml. You can use these variables in the YAML file as `${your_variable_name}`. |
| [testRunnerExecutor](#testrunnerexecutor) | String | To switch the test execution from powershell to the command line. |
|[workingDirectory](#workingdirectory)| Path of Location |This flag determines where test discovery and execution commands operate, including the creation of associated files or directories. |
| [captureScreenRecordingForScenarios](#capturescreenrecordingforscenarios) | bool |If this key is set to true, the video will be recorded for whole scenario execution |
| [buildConfig](#buildconfig) | Map | For managing the Hyperlink behaviour. |
| [matrixEnvPrefix](#matrixenvprefix) | Boolean |This flag is used to prefix env variables with `HE_ENV_` being set when you run a job in "matrix" mode. Default is false. |
| [dynamicAllocation](#dynamicallocation) | Boolean |This flag dynamically allocate test cases to workers as they become available and ensures that all available resources are used to run tests. |
| [collectLocalGitData](#collectlocalgitdata) | Boolean |This flag is used to store the git diff data related to the repository used to run HyperExecute Jobs. |
| [smartGrid](#smartgrid) | Boolean |When enabled, it will reduce the browser setup time in case of windows. |
| [scenarioCommandStatusOnly](#scenariocommandstatusonly) | Boolean |When enabled, it will mark the scenario as passed even if no test is associated with it or will mark the status of scenario based on the status of the last test executed. |
| [cypress](#cypress) | Boolean |This is required to be true in case user is running Cypress test suite. |
| [cypressOps](#cypressops) | Map |This map is used to pass cypress specific options. |  
| [shell](#shell) | String |Defines the shell on which all your commands should run. | 
| [dataJsonPath](#datajsonpath) | Array of String | DataJsonPaths helps to distribute data over VMs based on different Jsons.| 
| [dataJsonBuilder](#datajsonbuilder) | Array of String | dataJsonBuilder flag reads the data from your specified JSON files for distributed testing scenarios.| 
| [strict](#strict) | Boolean |If strict is set to true in yaml then the variables used in yaml must be present in either vars or environment variables on the user machine. | 
| [codeDirectory](#codedirectory) | String |Defines in which directory all of your commands defined in yaml would run. Your code would be downloaded in this directory. | 
| [preDirectives](#predirectives) | Map | Similar to pre with option to retry and specify shell. |
| [postDirectives](#postdirectives) | Map |Similar to post with option to retry. |
| [alwaysRunPostSteps](#alwaysrunpoststeps) | Boolean | Execute the Post Steps in every scenario case |
| [cacheTestURL](#cachetesturl) | Boolean |It enables users to cache static test files. |
| [project](#project) | String | Segregate data at a project/repo level |
| [slackChannel](#slackchannel) | String | Allows you to receive notifications of your Job updates directly in your preferred Slack channel. |
| [afterEachScenario](#aftereachscenario) | List of Strings | Allows you to trigger your define tasks after each test scenario. |
| [syncStart](#syncstart) | Map | Ensures all VMs are ready before starting parallel tasks.|
