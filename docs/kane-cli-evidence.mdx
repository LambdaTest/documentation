---
title: "Kane CLI Evidence Packs Explained"
sidebarTitle: "Overview"
description: "Every kane-cli run seals an evidence pack: a portable .evidence file with the test definition, per-step screenshots, console and network logs, and failures."
keywords: ['kane cli evidence', 'evidence pack', 'kane cli evidence pack', 'test run artifacts', 'kaneai', 'testmu ai']
"og:description": "Every kane-cli run seals an evidence pack: a portable .evidence file with the test definition, per-step screenshots, console and network logs, and failures."
---
...
| `kane-cli testrun run --remote` | Always. The suite's pack is sealed on the grid and downloaded into the store when the job ends, alongside the members' `output-<stem>/` recordings. See [Remote Runs](/docs/kane-cli-remote-execution/) |
