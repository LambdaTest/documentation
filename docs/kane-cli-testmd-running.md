...
sidebar_label: Running test.md Files
...
<script type="application/ld+json"
      dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "itemListElement": [{
          ...
          "@type": "ListItem",
          "position": 3,
          "name": "Running test.md files",
          "item": "https://www.testmuai.com/support/docs/kane-cli-testmd-running/"
        }]
      }) }}
></script>
...
Besides the output directory, every `testmd run` seals an [evidence pack](/support/docs/kane-cli-evidence/) — screenshots, per-step console/network logs, and failure records for the whole run — and copies it into `<cwd>/.testmuai/evidence/`. In a terminal you're offered to open it in the browser viewer at the end of the run; replayed runs also publish their pack to your project's execution history in Test Manager. Do **not** commit `.testmuai/evidence/` to git — packs are run artifacts, not test sources.
