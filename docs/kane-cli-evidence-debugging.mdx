---
title: "How to Debug a Failed Kane CLI Run"
sidebarTitle: "Debugging from a Pack"
description: "Use a kane-cli evidence pack to find why a run failed: the failed step, its console and network logs, the annotated screenshot, and the failed-vs-broken split."
keywords: ['debug failed test kane cli', 'evidence pack debugging', 'failure.yaml', 'failed vs broken', 'kaneai', 'testmu ai']
"og:description": "Use a kane-cli evidence pack to find why a run failed: the failed step, its console and network logs, the annotated screenshot, and the failed-vs-broken split."
---
