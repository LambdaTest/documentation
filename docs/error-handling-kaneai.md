...
:::note
A step that errors also flags the session: run actions on later steps are blocked and **Resume** is unavailable until you re-run, edit, or delete the errored step. See [The Error flag](/support/docs/kaneai-authoring-session/#the-error-flag).
:::
