...
| region | TYPE: STRING <br/> DEFAULT: Location of the nearest data center <br/> `region = US` | 1. The region capability allows users to select devices from a specific region for device allocation. To enable this feature, contact our Support team.<br/>2. By default, it picks the location of the nearest data center.<br/>3.  If you would like to change the region of the Data Center manually, you can pass this capability.<br/> 4. Currently, only three regions are supported: Europe - "EU", USA - "US" and Asia-Pacific - "AP" | <VirtualDeviceTag value="Virtual Device" />  <RealDeviceTag value="Real Device" /> |
...
| iosLiveInteraction | TYPE: BOOLEAN <br/> DEFAULT: False <br/>`true` | This capability allows users to interact with the iOS device during an automation session for scenarios requiring manual interaction.                                                                                                                            | <VirtualDeviceTag value="Virtual Device" />  <RealDeviceTag value="Real Device" /> |
