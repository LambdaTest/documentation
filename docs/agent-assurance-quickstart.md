...
:::note Current access
The <code>rook</code> CLI is currently distributed from a private GitHub repository. Your GitHub account must be allowed to read <code>LambdatestIncPrivate/rook</code>. This requirement is for downloading the packaged CLI, not for accessing the source during normal use.
:::
