...
sidebar_label: Test With Native App Scanner
...
Instead of always running the full built-in rule set, you choose a WCAG conformance target, turn rule groups on or off, and hand-pick individual rules. <BrandName /> remembers each user's choices and pre-fills them on the next scan.
