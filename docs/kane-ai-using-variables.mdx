...
### Using JSON Variables for Assertions

You can use JSON variables to perform detailed assertions on API responses, validating specific objects and elements within the response structure.
...
**Step 1: Initiate an API Call**
- Navigate to your desired API endpoint
- Configure the necessary API call parameters (HTTP method, headers, request body)
...
**Step 2: Access Response Variables**
...
Use double curly braces `{{` to access the variable list, navigate using arrow keys to select the status variable, and define your expected status code.
...
For example, if the expected status is 200 and the actual status is 500, the assertion will fail.
...
**Step 4: Assert Response Body**

Navigate through the response body using the same double curly brace method to browse to the specific JSON element you want to validate and create an assertion for that element.
...
You can check if specific elements exist in the response, verify property values, and validate nested JSON structures.
